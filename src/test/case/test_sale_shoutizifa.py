...
from src.test.page.sale.saleOrder_add_stzf_page import SaleOrderAddPage
from src.test.page.sale.saleTake_add_page import SaleTakeAddPage
    ...
    def test_saleOrder(self):
        ...
        for d in datas:
            with self.subTest(data=d):
                ...
                time.sleep(1)
                self.page.setting_order_ladingDirectDeliverySet_true()
                time.sleep(1)
                self.page.setting_order_orderLadingOnlyOne_true()
                ...
                self.page.get(self.URL + SaleOrderListPage().SALEORDERLISTPAGE_LIST_URL)
                time.sleep(1)
                self.page.sale_order_take_button()
                time.sleep(1)

                self.page.handle_switch()
                self.page = SaleTakeAddPage(self.page)
                time.sleep(1)
                self.page.sale_take_new()
                time.sleep(2)
...
if __name__ == '__main__':
    report  = REPORT_PATH + '\\test_sale_shoutizifa.html'
